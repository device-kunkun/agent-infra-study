...
如果之前的版本一直无法完成离线下载，更新后从后台彻底关闭 App 再打开，在设置中点“重新下载离线内容”。不需要删除主屏幕图标或清空学习记录；失败时页面会显示具体文件名与原因。
...
1.0.1 将构建时的原始首页嵌入 Service Worker，直接生成离线首页，避免托管服务的首页重定向或 HTML 注入造成误判。其他资源继续下载并按 SHA-256 校验；缺失、损坏、登录重定向或版本不符的文件不会被认定为完整下载。缓存版本同时包含资源与 Service Worker 代码的变化，修复缓存逻辑也会产生新版本。

“重新下载离线内容”会在联网时重新下载静态资源并验证完整性，保留学习记录。旧页面仍打开时不强制切换新缓存；关闭所有旧页面后再打开，才会接管已下载的新版本。

测试覆盖服务器改写或重定向首页、240 道题完整性、全部核心资源断网读取、下载错误反馈、损坏内容拒绝、缓存补全及不同路径的缓存隔离。测试在 Node 中模拟 Service Worker 环境，**不能替代真实 iPhone 的安装和断网冷启动验证**。
