...
const version=createHash('sha256').update(JSON.stringify(hashes)).update(sw).digest('hex').slice(0,16);
const shell=await readFile(path.join(root,'web','index.html'),'utf8');
sw=sw.replace('__BUILD_VERSION__',version).replace('__ASSET_HASHES__',JSON.stringify(hashes)).replace('__SHELL_HTML__',JSON.stringify(shell));
